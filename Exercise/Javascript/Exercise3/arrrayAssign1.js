...
const aWeekAgo = getDateSevenDaysAgo();

const activeUsers = users.filter(user => 
  user.posts.some(post => new Date(post.timestamp) > aWeekAgo)
)

const popularPosts = activeUsers.flatMap(user => 
  user.posts.filter(post => new Date(post.timestamp) > aWeekAgo && post.likes >= 10)
)

const averageLikesPerUser = popularPosts.reduce((sum, post) => sum + post.likes, 0) / activeUsers.length

console.log(activeUsers)
console.log(popularPosts)
console.log(`Average likes per active user: ${averageLikesPerUser.toFixed(2)}`)
